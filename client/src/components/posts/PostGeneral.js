...
let catPost = approvedPost.filter(function (post) {
  return post.catg == '0';
}).map(function (post) {
  return post;
})
  let filteredPosts = catPost.filter(post => post.text.toLowerCase().indexOf(search.toLowerCase()) !== -1);
...
const updateSearch = event =>
  ...
  return loading ? (
    ...
    <Fragment>
      ...
      {catPost.map(post => {<PostItem key={post._id} post={post} />})}
